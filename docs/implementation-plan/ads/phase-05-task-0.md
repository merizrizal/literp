...
Require `iat`, `exp`, and correct `aud` (string or string array containing the configured audience). Validate `nbf` when present. Proposed maximum token lifetime is five minutes, with at most 30 seconds clock skew; reject future issuance beyond skew and `exp <= iat`. Require provider access-token discriminator `typ=Bearer` in the signed claims for the approved Keycloak token profile; verify its actual provider representation at Chunk 0. ID tokens must fail even if otherwise cryptographically valid. Require a bounded, known `kid`; reject `none`, HS256, algorithm confusion, duplicate/ambiguous authorization headers, oversized tokens, and malformed claim types. Proposed header/token limit: 16 KiB; location grant limit: 256. These limits require approval against expected deployments.
